...
function CheckoutModal({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If M-Pesa payment, simulate STK push
    if (formData.paymentMethod === 'mpesa') {
      setMpesaPromptSent(true);
      // In production, call your M-Pesa API here
      await simulateMpesaPayment(formData.mpesaNumber, total);
    }
  ...
  // Simulate M-Pesa STK Push (In production, call actual API)
  const simulateMpesaPayment = async (phoneNumber: string, amount: number) => {
    console.log('M-Pesa STK Push initiated for:', phoneNumber, 'Amount:', amount);
    // In production, you would call your backend API that handles M-Pesa Daraja API
    /*
    const response = await fetch('/api/mpesa/stkpush', {
      ...
      body: JSON.stringify({
        ...
        transactionDesc: 'Fashion Purchase'
      })
    });
    */
  };
        ...
        <div className="relative bg-white rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
          ...
          {mpesaPromptSent && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-4">
              <p className="text-green-800 font-semibold mb-2">📱 M-Pesa Prompt Sent!</p>
              <p className="text-green-700 text-sm">Check your phone to complete payment</p>
            </div>
          )}
